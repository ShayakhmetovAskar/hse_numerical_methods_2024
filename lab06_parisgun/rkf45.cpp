...
namespace ADAAI {
std::vector<double> Timestepper::step(double t, const std::vector<double> &y,
                                      double &dt) {
  return {};
}

std::vector<double> HarmonicOscillator::f(double t,
  ...
  return {v_x, v_y, ax + a_coriolis_x - a_centrifugal * sin(alpha_rad),
          ay + a_coriolis_y - a_centrifugal * cos(alpha_rad),
          v_x / (R + y) / cos(alpha_rad)};
}

RKF45::RKF45(HarmonicOscillator *system_ptr) : system(system_ptr) {}
std::vector<double> RKF45::step(double t, const std::vector<double> &y,
                                double &dt) {
  ...
  if (dt < std::numeric_limits<double>::epsilon()) {
    std::cerr << "Warning: Time step became too small, integration might be "
                 "inaccurate."
              << std::endl;
  }
...
double MaxHeightRangeObserver::getMaxHeight() { return max_height; }

double MaxHeightRangeObserver::getMaxRange() { return max_range; }

Integrator::Integrator(Timestepper *timestepper, HarmonicOscillator *system,
                       Observer *observer)
    : timestepper(timestepper), system(system), observer(observer) {}

void Integrator::integrate(double t_start, const std::vector<double> &y_start,
...
int main() {
  std::vector<double> y0 = {0, 0, ADAAI::v_0 * cos(ADAAI::alpha_rad),
                            ADAAI::v_0 * sin(ADAAI::alpha_rad)};
