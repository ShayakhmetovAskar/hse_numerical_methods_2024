...
std::pair<double, double> SAM::operator()(double h) {
  ...
  if (h >= h0 && h <= h1) {
    Th = T0 - r0 * (h - h0);
    pressure =
        p0 * std::exp(g * std::log(1 - (r0 * (h - h0)) / T0) / (R_air * r0));
    ...
    Th = T2 - r2 * (h - h2);
    pressure =
        p2 * std::exp(g * std::log(1 - (r2 * (h - h2)) / T2) / (R_air * r2));
    density = pressure / (R_air * (T2 - r2 * (h - h2)));
    A = std::sqrt(pressure / density);
  } else {
    Th = T3 - r3 * (h - h3);
    pressure =
        p3 * std::exp(g * std::log(1 - (r3 * (h - h3)) / T3) / (R_air * r3));
...
int main() {
  ...
  std::cout << "Давление на высоте " << altitude << " м: " << result.first
            << " Па" << std::endl;
  std::cout << "Плотность на высоте " << altitude << " м: " << result.second
            << " кг/м^3" << std::endl;
  std::cout << '\n';
  std::cout << "Давление на высоте " << altitude
            << " м: " << ADAAI::SAM::get_pressure(altitude) << " Па"
            << std::endl;
  std::cout << "Плотность на высоте " << altitude
            << " м: " << ADAAI::SAM::get_density(altitude) << " Па"
            << std::endl;
  std::cout << "Скорость звука на высоте " << altitude
            << " м: " << ADAAI::SAM::get_A(altitude) << " Па" << std::endl;
