...
public:
  virtual std::vector<double> step(double t, const std::vector<double> &y,
                                   double &dt) = 0;
};

class HarmonicOscillator { // Class representing the harmonic oscillator
                           // (projectile motion)
public:
...
public:
  ...
  std::vector<double> step(double t, const std::vector<double> &y,
                           double &dt) override;
  ...
  static constexpr double b[6] = {16.0 / 135,      0,         6656.0 / 12825,
                                  28561.0 / 56430, -9.0 / 50, 2.0 / 55};
  static constexpr double b_star[6] = {25.0 / 216,    0,        1408.0 / 2565,
                                       2197.0 / 4104, -1.0 / 5, 0};
...
class MaxHeightRangeObserver
    : public Observer { // Example observer to print maximum height and range
public:
...
public:
  Integrator(Timestepper *timestepper, HarmonicOscillator *system,
             Observer *observer);

  void integrate(double t_start, const std::vector<double> &y_start,
